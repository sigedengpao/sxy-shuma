...
public class ProductDao {
	...
	public List<Object[]> getWeekHotProduct(User user) throws SQLException {
		List<Object[]> pList = searchBoughtKind(user);
		return pList;
	}
	public List<Object[]> searchBoughtKind(User user) throws SQLException {
		String sql = "SELECT products.id,products.name,products.imgurl,SUM(orderitem.buynum) totalsalnum FROM orders,products,orderItem WHERE orders.id=orderItem.order_id AND products.id=orderItem.product_id AND orders.paystate=1 and orders.user_id = ?"+
				" GROUP BY products.id,products.name,products.imgurl ORDER BY totalsalnum DESC LIMIT 0,2";
		QueryRunner runner = new QueryRunner(DataSourceUtils.getDataSource());
		return runner.query(sql, new ArrayListHandler(), user.getId());
	}
